...
def get_dss_data(primary_pathname_part_dss_filename_dict, primary_pathname_part, primary_part_c_part_dict=None, \
    ...
    for pp in primary_pathname_part_dss_filename_dict:
        ...
        if primary_pathname_part is 'b_part':
            b_part=pp
            c_part = primary_part_c_part_dict[pp] if primary_part_c_part_dict is not None else None
            e_part = primary_part_e_part_dict[pp] if primary_part_e_part_dict is not None else None
        ...
        with pyhecdss.DSSFile(dss_filename) as d:
            ...
            for p in path_list:
                ...
                if isinstance(df.index, pd.core.indexes.datetimes.DatetimeIndex):
                    print('timeseries is inst-val, converting to per-aver. b_part, c_part='+str(b_part)+','+str(c_part))
                ...
                if primary_pathname_part is 'b_part':
                    df.columns=[pp]
                print('path='+p)
                return_df = df if return_df is None else pd.merge(return_df, df, how='left', left_index=True, right_index=True)
            d.close()
        print('==============================================================')
...
def get_file_path(selected_files_dict, row_index, text_field_text_var, title, filetypes, initialdir, initialfile, file_not_folder=True):
    '''
    ...
    file_path = None
    if file_not_folder:
        file_path = tk.filedialog.askopenfilename(title=title, filetypes=filetypes, initialdir=initialdir.get(), initialfile=initialfile)
    else:
...
def add_file_or_folder_specifier(root, selected_files_dict, row_index, title, filetypes, initialdir, initialfile, file_not_folder=True):
    '''
    ...
    button=ttk.Button(root, text=title, command=lambda:get_file_path(selected_files_dict, row_index, text_field_text_var, title, filetypes, initialdir, initialfile, file_not_folder=file_not_folder))
...
def create_ann_inputs():
    ...
    model_input_folder = base_study_folder + 'dsm2_2022.01_historical_update/timeseries/'
    model_output_folder = base_study_folder + 'dsm2_2022.01_historical_update/studies/historical/output/'
    ...
    cwd.set(model_input_folder) if model_data_version=='2021' else cwd.set('D:/delta/dsm2_v8.2.1_historical/timeseries2019/')

    selected_files_dict = {}

    add_file_or_folder_specifier(root, selected_files_dict, 0, 'boundary input', dss_filetypes, cwd, 'hist.dss')
    add_file_or_folder_specifier(root, selected_files_dict, 1, 'gate input', dss_filetypes, cwd, 'gates-v8.dss')
    add_file_or_folder_specifier(root, selected_files_dict, 2, 'DCD input', dss_filetypes, cwd, 'DCD_hist_Lch5.dss')
    add_file_or_folder_specifier(root, selected_files_dict, 3, 'SMCD input', dss_filetypes, cwd, 'SMCD_hist.dss')
    cwd.set(model_output_folder)
    add_file_or_folder_specifier(root, selected_files_dict, 4, 'EC model output', dss_filetypes, cwd, 'hist_v2022_01_EC.dss')
    cwd.set(output_folder)
    add_file_or_folder_specifier(root, selected_files_dict, 5, 'output folder', None, cwd, '', file_not_folder=False)
    # Button for closing
    proceed_booleanvar=tk.BooleanVar(False)
    submit_button = ttk.Button(root, text="Create ANN Input", command=lambda:submit_listener(root, proceed_booleanvar))
